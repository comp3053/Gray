...
public class StorageIngredient extends Ingredient {
	...
	// Getting all storage ingredient and their information
	...
	// Checking for updating status, model 1 is for adding, model 0 is for subtracting
	public String checkForUpdating(String maltT,String hopT,String yeastT,String sugarT,String additiveT,int model) throws SQLException {
		...
		// Get the current amount of ingredient in the database
		...
		// Below is to check whether to set the new value to adding or subtracting
		if(maltT.length()!=0) {
			try{
		...
		// Below operation is the same as above one
		if(hopT.length()!=0) {
			try{
		...
		// Do the update for each ingredient
